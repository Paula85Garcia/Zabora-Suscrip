...
/**
 * Servicio de Pagos con integracion completa de Stripe
 * Maneja creacion de Payment Intents, procesamiento de webhooks y estados de pago
 */
@Service
@Slf4j
...
public class PagoServicioReal {
    ...
    /**
     * Crear un Payment Intent real en Stripe
     * Retorna el client secret para integracion con frontend
     */
    @Transactional
    public RespuestaPagoDTO crearIntentoPago(String usuarioId, SolicitudPagoDTO solicitud) {
        ...
        Optional<UsuarioSuscripcion> suscripcionOpt = 
            suscripcionRepository.findById(solicitud.getIdSuscripcion());
        ...
        try {
            ...
            long amountInCents = solicitud.getMonto()
            ...
            PaymentIntentCreateParams.Builder paramsBuilder = PaymentIntentCreateParams.builder()
                ...
                .setCurrency("cop")
                .setDescription("Suscripcion Premium - Zabora")
                .putMetadata("suscripcion_id", solicitud.getIdSuscripcion())
                .putMetadata("usuario_id", usuarioId)
            ...
            return RespuestaPagoDTO.builder()
                .exito(true)
                .mensaje("Payment Intent creado exitosamente - proceder con Stripe Elements")
                .idPago(pago.getId())
                .estado("PENDIENTE")
                ...
                .recibirFactura(solicitud.getRecibirFactura())
                .detalles(detalles)
                .build();
    ...
    public void handlePaymentSucceeded(PaymentIntent paymentIntent) {
        ...
        try {
            ...
            log.info("Pago procesado exitosamente - Suscripcion activada: {}", 
                     suscripcion.getId());
    ...
    public Map<String, Object> obtenerEstadoPago(String paymentIntentId) {
        try {
            ...
            pagoRepository.findByIdIntentoPagoStripe(paymentIntentId)
                .ifPresent(pago -> {
    ...
    public List<Map<String, Object>> obtenerHistorialPagos(String usuarioId) {
        return pagoRepository.findByUsuarioIdOrderByFechaCreacionDesc(usuarioId).stream()
            .map(pago -> {
                ...
                pagoMap.put("fecha", pago.getFechaPago() != null ? 
                          pago.getFechaPago() : pago.getFechaCreacion());
                pagoMap.put("metodo_pago", pago.getMetodoPago().name());
                pagoMap.put("url_comprobante", 
                          pago.getUrlComprobante() != null ? pago.getUrlComprobante() : "");
                return pagoMap;
            })
